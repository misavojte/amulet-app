<script lang="ts">
	...
	import Coin from './GameCoin.svelte';
...
<div
	class="box"
	style="{cssVarStyles};transition-delay:{delay}ms"
	class:hide={$gameState.gameStage === 'AmuletDecision'}
	class:block={$gameState.blockInteraction}
	on:click={triggerOpenTop}
>
	...
	<div
		class="top"
		class:open={openTop}
		style:transition="transform ease-out {OPEN_TOP_TRANSITION_DURATION}ms"
	></div>
	{#if showContent}
		<div class="content">
			<Coin />
...
<style>
	...
	.content {
		transform: translateZ(calc(var(--zLength) * 0.75)) rotateZ(var(--zRotationCompensation))
			rotateX(var(--xRotationCompensation));
	...
	@keyframes animateCoin {
		...
		30% {
			transform: translateZ(300px) translateY(10px) rotateZ(var(--zRotationCompensation))
				rotateX(var(--xRotationCompensation));
		}
		100% {
			transform: translateZ(500px) translateY(500px) rotateZ(var(--zRotationCompensation))
				rotateX(var(--xRotationCompensation));
		}
	}
</style>
