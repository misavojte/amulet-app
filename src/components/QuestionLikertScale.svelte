...
<Question questionText={question.headingText}>
	<div class="flex justify-center flex-col w-full">
		...
		<div
			class="grid grid-cols-3 gap-4 justify-between mx-auto w-full text-neutral-500 font-thin text-md"
			style="max-width: {labelContainerMaxWidth};"
		>
			<div class="text-left text-wrap leading-tight">{question.label.min}</div>
			<div class="text-center text-wrap leading-tight">{question.label.avg}</div>
			<div class="text-right text-wrap leading-tight">{question.label.max}</div>
		</div>
	</div>
</Question>
